...
def train_one_run(
    ...
    print(
        ...
        f"{format_metric(summary['test_first_decision_accuracy_at_threshold'])} "
        f"coverage={format_metric(summary['test_first_decision_coverage_at_threshold'])} "
        f"mean_prefix={format_metric(summary['test_mean_first_decision_visible_ratio_at_threshold'])} "
...
def write_summary(output_dir: Path, summaries: list[dict[str, object]]) -> None:
    ...
    for row in sorted_summaries:
        lines.append(
            f"| {row['run_name']} | {format_metric(row['summary_threshold'], 2)} | "
            f"{format_metric(row['test_first_decision_accuracy_at_threshold'])} | "
            f"{format_metric(row['test_first_decision_coverage_at_threshold'])} | "
            f"{format_metric(row['test_first_decision_correct_coverage_at_threshold'])} | "
            f"{format_metric(row['test_mean_first_decision_visible_ratio_at_threshold'])} | "
            f"{format_metric(row['test_forced_final_accuracy'])} |"
        )
